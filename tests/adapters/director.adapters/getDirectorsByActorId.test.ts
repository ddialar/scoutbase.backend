...
import { getDirectorsByActorId } from '@adapters';
...
describe('Testing Director adapters ...', () => {
    describe('working with \'getDirectorsByActorId\' ...', () => {
        test('with no actor ID provided, it must return an empty collection.', () => {
            ...
            expect(getDirectorsByActorId(actorId)).rejects.toThrow('Actor identification not valid.');
        });
        test('providing a non recorded actor ID, it must return an empty collection.', async (done) => {
            let actorId = 99;
            let expectedResult = [];
            let obtainedResult = await getDirectorsByActorId(actorId);
