...
import { updateUserToken } from '@adapters';
import { 
    NewUserInterface,
    UserInterface
} from '@interfaces';
...
describe('Testing User adapters ...', () => {
    describe('working with \'updateUserToken\' ...', () => {
        test('with no userId provided, it must throw an error.', () => {
            ...
            expect(updateUserToken(userId, newToken)).rejects.toThrow('User identification not valid.');
        });
        test('providing a non recorded userId, it must return a NULL value.', async (done) => {
            let userId: number = 99;
            let newToken: string = 'newUserToken';
            let obtainedResult = await updateUserToken(userId, newToken);
