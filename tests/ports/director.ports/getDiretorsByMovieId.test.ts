...
describe('Testing Director ports ...', () => {
    describe('working with \'getDirectorsByMovieId\' ...', () => {
        test('with no movie ID provided, it must return an empty collection.', () => {
            ...
            expect(getDirectorsByMovieId(movieId)).rejects.toThrow('Movie identification not valid.');
        });
        test('providing a non recorded movie ID, it must return an empty collection.', async (done) => {
            let movieId = 99;
            let expectedResult = [];
            let obtainedResult = await getDirectorsByMovieId(movieId);
